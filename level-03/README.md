...
In assenza dell'indizio nel commento HTML, la cartella `/s3cr3t/` avrebbe potuto essere scoperta tramite **directory brute forcing**: si utilizza un tool (come `gobuster` o `feroxbuster`) che prova in automatico centinaia o migliaia di nomi di cartelle comuni contro il server, inserendoli nell'URL uno per volta e osservando quali rispondono con un codice 200 o 301 anziché 404. Il tool legge i nomi da una **wordlist** — una lista di percorsi comuni usata come dizionario. Questa tecnica non richiede indizi nel sorgente e funziona anche quando `robots.txt` non elenca nulla di utile.
